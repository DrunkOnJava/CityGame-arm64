...
@property (nonatomic, assign) EnhancedCityStats cityStats;
...
@property (nonatomic, strong) NSPopUpButton *categorySelector;
@property (nonatomic, strong) NSPopUpButton *buildingSelector;
...
- (instancetype)initWithFrame:(NSRect)frame {
    ...
    if (self) {
        self.gridSize = 32; // Good size for demo
...
- (void)createEnhancedSpriteAtlas {
    // Create a colorful atlas representing different building types
    int atlasSize = 2048;
...
- (void)initializeCityData {
    ...
    // Add diverse starter buildings showcasing different categories
    ...
    [self setTileAt:30 y:20 type:TILE_POWER_PLANT];
    [self setTileAt:30 y:22 type:TILE_SOLAR_PANEL];
    [self setTileAt:6 y:6 type:TILE_CINEMA];
    [self setTileAt:22 y:22 type:TILE_STADIUM];
    [self setTileAt:26 y:26 type:TILE_AIRPORT];
    
    NSLog(@"✅ Enhanced city data initialized: %dx%d with diverse buildings", self.gridSize, self.gridSize);
}
...
- (void)createEnhancedUI {
    ...
    self.categorySelector = [[NSPopUpButton alloc] initWithFrame:NSMakeRect(10, 10, 150, 25)];
    for (NSString *name in categoryNames) {
        [self.categorySelector addItemWithTitle:name];
    }
    [self.categorySelector setTarget:self];
    [self.categorySelector setAction:@selector(categoryChanged:)];
    [self addSubview:self.categorySelector];
    
    // Building selector
    self.buildingSelector = [[NSPopUpButton alloc] initWithFrame:NSMakeRect(170, 10, 200, 25)];
    [self.buildingSelector setTarget:self];
    [self.buildingSelector setAction:@selector(buildingChanged:)];
    [self addSubview:self.buildingSelector];
    
    // Enhanced statistics label
    self.statsLabel = [[NSTextField alloc] initWithFrame:NSMakeRect(10, 200, 350, 150)];
    ...
    // Time display
    self.timeLabel = [[NSTextField alloc] initWithFrame:NSMakeRect(10, 360, 200, 40)];
    ...
    // Pause button
    self.pauseButton = [[NSButton alloc] initWithFrame:NSMakeRect(220, 360, 80, 40)];
    ...
    // Building type label
    self.buildingTypeLabel = [[NSTextField alloc] initWithFrame:NSMakeRect(10, 40, 500, 20)];
...
- (void)categoryChanged:(NSPopUpButton*)sender {
    self.currentCategory = (BuildingCategory)sender.indexOfSelectedItem;
    [self updateBuildingOptions];
    [self updateBuildingTypeLabel];
}

- (void)buildingChanged:(NSPopUpButton*)sender {
    // Find the building type based on category and selection
    int selectionIndex = 0;
    for (int i = 0; i < TILE_TYPE_COUNT; i++) {
        if (buildingProperties[i].category == self.currentCategory) {
            if (selectionIndex == sender.indexOfSelectedItem) {
...
- (void)updateBuildingOptions {
    ...
    [self.buildingSelector removeAllItems];
    
    // Add buildings in current category
    for (int i = 0; i < TILE_TYPE_COUNT; i++) {
        if (buildingProperties[i].category == self.currentCategory) {
            NSString *name = [NSString stringWithUTF8String:buildingProperties[i].name];
            [self.buildingSelector addItemWithTitle:name];
        }
    }
    ...
    [self.buildingSelector selectItemAtIndex:0];
}

- (void)togglePause:(NSButton*)sender {
    self.gamePaused = !self.gamePaused;
    [self.pauseButton setTitle:self.gamePaused ? @"▶️ Play" : @"⏸️ Pause"];
}
...
- (void)updateStats {
    // Reset statistics
    memset(&_cityStats, 0, sizeof(EnhancedCityStats));
    _cityStats.city_funds = 50000; // Starting funds
    ...
    for (int y = 0; y < self.gridSize; y++) {
        for (int x = 0; x < self.gridSize; x++) {
            ...
            _cityStats.population += props.population_capacity;
            _cityStats.monthly_income += props.monthly_income;
            _cityStats.monthly_expenses += props.monthly_expense;
            
            // Count by category
            switch (props.category) {
                case CATEGORY_RESIDENTIAL: _cityStats.residential_buildings++; break;
                case CATEGORY_COMMERCIAL: _cityStats.commercial_buildings++; break;
                case CATEGORY_INDUSTRIAL: _cityStats.industrial_buildings++; break;
                case CATEGORY_SERVICES: _cityStats.service_buildings++; break;
                case CATEGORY_UTILITIES: _cityStats.utility_buildings++; break;
                default: break;
            }
            
            // Calculate coverage and efficiency
            if (type == TILE_HOSPITAL) _cityStats.hospital_coverage += 15;
            if (type == TILE_POLICE_STATION) _cityStats.police_coverage += 12;
            if (type == TILE_FIRE_STATION) _cityStats.fire_coverage += 10;
            if (type == TILE_SCHOOL || type == TILE_LIBRARY) _cityStats.education_coverage += 8;
            if (type == TILE_POWER_PLANT) _cityStats.power_capacity += 1000;
            if (type == TILE_SOLAR_PANEL) _cityStats.power_capacity += 100;
            if (type == TILE_WATER_TOWER) _cityStats.water_capacity += 500;
        }
    }
    
    // Calculate derived metrics
    int totalBuildings = _cityStats.residential_buildings + _cityStats.commercial_buildings + 
                        _cityStats.industrial_buildings + _cityStats.service_buildings;
    
    if (totalBuildings > 0) {
        _cityStats.happiness = 50 + (_cityStats.service_buildings * 5) - (_cityStats.industrial_buildings * 2);
        _cityStats.happiness = MAX(0, MIN(100, _cityStats.happiness));
        
        _cityStats.safety_rating = MIN(100, _cityStats.police_coverage + _cityStats.fire_coverage);
        _cityStats.environmental_score = MAX(0, 50 + (_cityStats.utility_buildings * 2) - (_cityStats.industrial_buildings * 3));
    }
    
    _cityStats.power_usage = totalBuildings * 10;
    _cityStats.water_usage = _cityStats.population * 2;
    _cityStats.employment_rate = _cityStats.population > 0 ? MIN(100, (_cityStats.monthly_income / 10) * 100 / _cityStats.population) : 0;
    ...
    NSString *statsText = [NSString stringWithFormat:
        ...
        @"Industrial: %d\n"
        @"Services: %d",
        _cityStats.population,
        _cityStats.employment_rate,
        _cityStats.happiness,
        _cityStats.safety_rating,
        _cityStats.environmental_score,
        _cityStats.city_funds,
        _cityStats.monthly_income,
        _cityStats.monthly_expenses,
        _cityStats.monthly_income - _cityStats.monthly_expenses,
        _cityStats.residential_buildings,
        _cityStats.commercial_buildings,
        _cityStats.industrial_buildings,
        _cityStats.service_buildings
    ];
...
- (void)mouseDown:(NSEvent *)event {
    ...
    if (tileX >= 0 && tileX < self.gridSize && tileY >= 0 && tileY < self.gridSize) {
        if ([event buttonNumber] == 0) { // Left click - place building
            ...
            BuildingProperties props = buildingProperties[self.currentBuildingType];
            if (_cityStats.city_funds >= props.cost) {
                [self setTileAt:tileX y:tileY type:self.currentBuildingType];
                _cityStats.city_funds -= props.cost;
                [self rebuildGeometry];
                [self updateStats];
            } else {
                NSLog(@"💰 Not enough funds! Need $%d, have $%d", props.cost, _cityStats.city_funds);
            }
            ...
            if (currentType != TILE_EMPTY) {
                ...
                _cityStats.city_funds += props.cost / 2; // 50% refund
                [self rebuildGeometry];
                [self updateStats];
            }
        }
...
- (void)mtkView:(MTKView *)view drawableSizeWillChange:(CGSize)size {
    ...
    self.statsLabel.frame = NSMakeRect(10, size.height - 180, 350, 150);
    self.timeLabel.frame = NSMakeRect(10, size.height - 210, 200, 40);
    self.pauseButton.frame = NSMakeRect(220, size.height - 210, 80, 40);
}
...
- (void)applicationDidFinishLaunching:(NSNotification *)notification {
    ...
    self.window.title = @"🏙️ SimCity ARM64 - Integrated with Enhanced 3D Assets";
    ...
    NSLog(@"🚀 Integrated SimCity launched with all agent work and 3D assets!");
    NSLog(@"🎮 Enhanced Controls:");
    NSLog(@"   • Category dropdown: Select building category");
    NSLog(@"   • Building dropdown: Select specific building type");
    NSLog(@"   • Left click: Place building");
    NSLog(@"   • Right click: Remove building (50%% refund)");
    NSLog(@"   • Space: Pause/unpause time");
    NSLog(@"   • +/-: Change time speed");
    NSLog(@"   • Drag to pan camera");
    NSLog(@"   • Scroll to zoom");
}
