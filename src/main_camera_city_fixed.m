...
- (instancetype)initWithDevice:(id<MTLDevice>)device {
    ...
    if (self) {
        ...
        self.uniformBuffer = [device newBufferWithLength:sizeof(Uniforms)
...
- (void)setupPipeline {
    ...
    NSString *shaderSource = @"#include <metal_stdlib>\n"
        ...
        "    worldPos.x -= uniforms.camera_x;\n"
        "    worldPos.z -= uniforms.camera_z;\n"
        "    \n"
        ...
        "    float scale = 50.0 / uniforms.camera_height;\n"
...
- (void)drawInMTKView:(MTKView *)view {
    ...
    // Update uniform buffer with camera state
    Uniforms *uniforms = (Uniforms *)[self.uniformBuffer contents];
    uniforms->camera_x = camera_state.world_x;
    uniforms->camera_z = camera_state.world_z;
    uniforms->camera_height = camera_state.height;
    ...
    // Take screenshot
    if (self.frameCount == 300) { // 5 seconds to move camera around
    ...
    if (self.frameCount % 60 == 0) {
        printf("Camera: World(%.1f, %.1f) Height:%.1f Vel:(%.2f, %.2f) Keys:0x%x\n",
               camera_state.world_x, camera_state.world_z, camera_state.height,
               camera_state.vel_x, camera_state.vel_z, g_input.keys);
    }
}
...
- (void)captureScreenshot:(MTKView *)view {
    ...
    [pngData writeToFile:@"camera_city_fixed_screenshot.png" atomically:YES];
    printf("📸 Screenshot saved as camera_city_fixed_screenshot.png\n");
...
- (void)mouseMoved:(NSEvent *)event {
    ...
    if (g_input.mouse_x < 15 || g_input.mouse_x > self.frame.size.width - 15 ||
        g_input.mouse_y < 15 || g_input.mouse_y > self.frame.size.height - 15) {
        // Edge panning is handled in the assembly code
    }
}
...
- (void)applicationDidFinishLaunching:(NSNotification *)notification {
    ...
    [self.window setTitle:@"SimCity ARM64 - Camera Controller Demo (Move with WASD!)"];
    ...
    printf("Window opened! Use WASD to move, mouse wheel to zoom.\n");
}
